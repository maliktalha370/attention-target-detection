...
class Inference:
    ...
    def run(self,frame_raw, head_box):
...
if __name__ == "__main__":
    ...
    IMAGE_DIR = 'data/gaze_follow_test/frames'
    HEAD = 'data/gaze_follow_test/test.txt'
    ...
    for i in df.index:
        ...
        starting_point, ending_point = obj.run(frame_raw, head_box)
        img_cp = np.array(frame_raw)
        if starting_point != None and ending_point != None:

            cv2.line(img_cp, starting_point, ending_point, (255, 0, 0), 5)
            cv2.circle(img_cp,ending_point, 15,
                       (255, 0, 0), -1)
